...
model RLPPOV1
  "Variable air volume flow system with terminal reheat and five thermal zones using PPO Control"
...
equation
  connect(TSupSetHea, heaCoiCon.u_s) annotation (Line(
  ...
  connect(TMix.T, conEco.TMix) annotation (Line(
      ...
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(conEco.TSupHeaSet, TSupSetHea) annotation (Line(
  ...
  connect(TSetCoo.TSet, conEco.TSupCooSet) annotation (Line(
      ...
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(TSupSetHea, TSetCoo.TSetHea) annotation (Line(
